...
# --- ESKİ HALİ ---
# def optimize_route(activity_ids: list):
#     start = activity_ids[0] 

# --- YENİ HALİ (Bunu kopyala) ---
def optimize_route(activity_ids: list, user_coords: tuple = None) -> dict:
    """
    Kullanıcının anlık konumundan (user_coords) başlayarak 
    en yakın durakları bulan optimize edilmiş rota.
    """
    if not activity_ids:
        return {"route": [], "total_distance_km": 0}

    # Eğer GPS verisi gelmezse (örn. Mac'ten konum alınamadı), 
    # varsayılan olarak Antalya Saat Kulesi'ni baz alalım.
    if user_coords is None:
        user_coords = (36.8865, 30.7047) 

    graph = build_graph(activity_ids)
    
    # Rota artık kullanıcının olduğu yerden başlıyor
    route = []
    current_coords = user_coords
    unvisited = set(activity_ids)
    total_distance = 0
    segments = []

    while unvisited:
        # Mevcut konuma (current_coords) en yakın olan aktiviteyi bul
        nearest = min(
            unvisited,
            key=lambda x: haversine_distance(current_coords, COORDINATES.get(x))
        )
        
        dist = haversine_distance(current_coords, COORDINATES.get(nearest))
        total_distance += dist
        ...
        current_coords = COORDINATES.get(nearest) # Yeni konumumuz artık bu aktivite

    # ... (Geri kalan isimlendirme kısımları aynı kalabilir)
